...
class MissionPlanner(Process):
    ...
    def __init__(
        ...
        self._queues_dir.register(queue=self._events_q, name=self._events_q_name)
    ...
    def set_new_mission(
        ...
        if mission is None:
            mission = Mission(
                home=home, waypoints=waypoints, speed_limits=speed_limits, armed=arm
            )

        event = Event(
            source=MissionPlanner.event_source_name,
            destination=MissionPlanner.event_q_name,
    ...
    def _send_mission_to_afcs(self, mission: Mission):
        ...
        try:
            ...
            event = Event(
                ...
                destination=MISSION_SENDER_QUEUE_NAME,
                operation="post_mission",
                parameters=mission,
            )
            afcs_q.put(event)
        except Exception as e:
            self._log_message(LOG_ERROR, f"ошибка отправки миссии по mqtt: {e}")
    ...
    def _send_mission_to_communication_gateway(self):
        ...
        communication_q: Queue = self._queues_dir.get_queue(communication_q_name)
        try:
